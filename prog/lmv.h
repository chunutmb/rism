    ...
    double **invwc1w, uv_t *uv, int *niter)
{
  ...
  step_picard(model, NULL, vrsr, wk, cr, ck, vklr, tr, tk, NULL, 0);
  ...
  for ( it = 0; it < model->itmax; it++ ) {
    ...
    err = closure(model, NULL, NULL, vrsr, cr, tr1, uv->prmask, 0.0);
    if ( err < errmin ) {
      cparr2d(crbest, cr, ns * ns, npt);
      errmin = err;
    }
    ...
    closure(model, NULL, der, vrsr, cr, tr, uv->prmask, 1.0);
  ...
  err = step_picard(model, NULL, vrsr, wk, cr, ck, vklr, tr, tk, uv->prmask, 0);
