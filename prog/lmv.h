    ...
    uv_t *uv, int *niter)
{
  ...
  step_picard(model, NULL, vrsr, wk, cr, ck, vklr, tr, tk, uv->prmask, 0.);
  ...
  for ( it = 0; it <= model->itmax; it++ ) {
    ...
    err = closure(model, NULL, NULL, vrsr, cr, lmv->tr1, uv->prmask, 0.);
    lmv_savebest(lmv, cr, err);

    /* compute c(r) and c(k) from the closure */
    closure(model, NULL, lmv->der, vrsr, cr, tr, uv->prmask, 1.);
