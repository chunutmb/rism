    ...
    for ( j = i; j < ns; j++, ipr++ ) { /* the second site */
      ...
      if ( !once )
        fprintf(stderr, "i %d, j %d, sig %4.2f, eps6 %10.2e, eps12 %10.2e, "
            "c6 %10.2e c12 %10.2e, B %10.2e, rho %4.2f, rmin %g\n",
          i + IDBASE, j + IDBASE, sig, eps6, eps12, c6, c12, Bij, rhoij, rminij);
    ...
    int *prmask, double damp)
{
  ...
  return closure(model, res, NULL, vrsr, cr, tr, prmask, damp);
}
...
#define step_uu_infdil_atomicsolute(m, vrsr, wk, cr, ck, \
    vklr, tr, tk, prmask) \
  step_picard(m, NULL, vrsr, wk, cr, ck, vklr, tr, tk, prmask, 1.0)
